...
public class SocialPrefsUtil {
	...
	public static SocialPrefs createSocialPrefs(HttpServletRequest request,
		...
		if (StringUtils.isBlank(signupType)
				|| StringUtils.equalsIgnoreCase(signupType, "reviewdesk")
				|| user == null)
			return null;
	...
	private static SocialPrefs createFacebookSocialPrefs(
			HttpServletRequest request) throws Exception {
		// TODO Auto-generated method stub

		// Get facebook user details from session
		String facebookUserDetails = (String) request
				.getSession()
				.getAttribute(ScribeOpenIdServlet.SERVICE_TYPE_FACEBOOK_SESSION);

		System.out.println("facebookUserDetails = from session = "
				+ facebookUserDetails);
		...
		facebookUserDetailJSON.put("id",
				JSONUtil.getJSONValue(facebookJSON, "id"));
		facebookUserDetailJSON.put("name",
				JSONUtil.getJSONValue(facebookJSON, "name"));
		facebookUserDetailJSON.put("email",
				JSONUtil.getJSONValue(facebookJSON, "email"));

		String longLivedAccessToken = FacebookUtil
				.getExtendedPermissionAccessToken(JSONUtil.getJSONValue(
						facebookJSON, "access_token"));
		...
		if (StringUtils.isBlank(longLivedAccessToken)) {
			...
			longLivedAccessToken = JSONUtil.getJSONValue(facebookJSON,
					"access_token");
		...
		System.out.println("facebookUserDetailJSON = while registering"
				+ facebookUserDetailJSON);

		facebookUserDetailJSON.put("expiry", expiry + "");

		System.out
				.println("facebookUserDetailJSON = " + facebookUserDetailJSON);
		...
		if (StringUtils.isBlank(facebookSelectedPageId)
				|| pageDetailsJsonArray == null
				|| pageDetailsJsonArray.length() == 0) {
		...
		for (int i = 0; i < pageDetailsJsonArray.length(); i++) {
			...
			if (StringUtils.isNotBlank(id)
					&& StringUtils.equalsIgnoreCase(id, facebookSelectedPageId))
				facebookPagesJSON.put(facebookSelectedPageId,
						JSONUtil.getJSONValue(eachPageJSON, "name"));
